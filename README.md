...
Все новые CSS фитчи помечены `новая фитча css` тегом
...
То как накладывать елементы друг на друга так чтобы внешне не оставалось фантомного пустого следа. Мы можем просто воспользоваться негативной маржой - `margin-bottom: -1px; /* Move tabs 1px lower */`. Если же нужен фантомный пустой след, то нежно:
```
  position: relative;
  top: 10px;
```
...
(родитель обязательно должен быть 'display: flex;')
```
align-self: flex-start/end/center/stretch; 
OR margin-top: auto;
```

`margin: 0 auto;` - самоцентровка родителя
`justify-content: center;` - центрирует детей
...
`flex: 1 1 auto;` - это сокращенная запись трех свойств и может быть записано так:
```
flex-grow: 1; - показывает, может ли расти флекс-элемент и насколько
flex-shrink: 1; - показывает, может ли уменьшаться флекс-элемент и насколько
flex-basis: auto; - показывает исходный (базовый размер/ширину) элемента, относительно которых уже он увеличивается или уменьшается.
```

Отображение контента на весь екран для мобильных устройств (новая фитча css):  
height: 100svh/100lvh (small/large viewport height) - чтобы полная высота экрана отображалась корректно на телефонах  
height: 100dvh (dynamic viewport height) - динамически подстраивается под меняющуюся высоту экрана телефона при скролле

Новый способ центровки елементов без flex (новая фитча css):  
`align-content: start;`
...
Задать пропрорции размера елемента. Нужно скомбенировать aspect-ratio с каким-либо размером:
```
aspect-ratio: 16 / 9;
width: 50%
```
